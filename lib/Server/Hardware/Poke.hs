...
-- request.
type SubmitPoke = CogName -> Vector Text -> F.Fan -> IO ()

type RequestStorage = Map (CogName, Vector Text) (MinHeap Int)
...
addRequest :: CogName -> Vector Text -> Int -> RequestStorage -> RequestStorage
...
rmRequest :: CogName -> Vector Text -> Int -> RequestStorage -> RequestStorage
  ...
  case toList syscall.args of
    ["poke", ROW nats] -> do
      ...
      let docancel = do
            ...
            modifyTVar' st.requests (rmRequest syscall.cog path key)
...
submitPoke st cogName path val = do
  ...
  w <- newProcessAsync "Poke" "Poke" $
    withTracingResultsFlow "submitPoke" "poke" $ atomically do
      ...
      let myb = lookup (cogName, path) tv
