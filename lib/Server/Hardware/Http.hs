...
stopCogById :: HWState -> IO ()
stopCogById st = cancelCog =<< readTVarIO st.cogs

spinCog :: Debug => FilePath -> LmdbStore -> WS.ServerApp -> Client.Manager -> IO CogState
spinCog mach store wsApp manager = do
    traceM "HTTP_SPINNING"
    let localhost = N.tupleToHostAddress (0x7f, 0, 0, 1)
    let flags = [N.AI_NUMERICHOST, N.AI_NUMERICSERV]
    let tcp   = 6
    let addr  = (N.SockAddrInet 808080 localhost)
    let ainfo = N.AddrInfo flags N.AF_INET N.Stream tcp addr Nothing
    listenSocket <- N.openSocket ainfo
    N.listen listenSocket 5 -- TODO Should this be 5?
    listenPort <- fromIntegral <$> N.socketPort listenSocket
    ...
    let baseName  = "http.port"
    let portFile = mach </> baseName
    ...
    mdo
        serverThread <- async (servThread store wsApp cs)
        clientThread <- async (clientWorker manager cs)
        let file = portFile
...
createHardwareHttp mach store wsApp = do
  ...
  where
    ...
    startup = do
        ...
        cogs <- newTVarIO =<< spinCog mach store wsApp manager
